...
含有纯虚函数的类，或继承但未覆盖的类，是抽象基类。不能直接创建抽象类的对象。
...
Just as friendship is not transitive (§7.3.4, p. 279), friendship is also not inherited. 基类的友元对子类的成员没有特殊的权限，子类的友元对基类也没有特殊的权限。
