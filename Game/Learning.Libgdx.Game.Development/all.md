...
Libgdx comes with a great feature set to easily create **scene graphs**. A scene graph is a hierarchically organized structure of objects similar to files and folder. 在Libgdx中这些对象称为actors。Actors can be **nested** to create logical groups. 分组很强大，对父actor的修改也会影响它的子actors。每个actor都自己坐标系统。
...
requirements are.
Since the user interface of the menu that we are going to create is rather complex, 
we want to make use of Libgdx's scene graph for this task. More precisely, we will 
use Scene2D UI. This is another implementation in Libgdx that builds on top of 
Scene2D and extends its functionality by providing a rich set of common and 
ready-to-use UI elements. In Libgdx, these UI elements are called widgets.
The following is a brief list of all the widgets currently available in Scene2D UI:
Button, CheckBox, Dialog, Image, ImageButton, Label, List, ScrollPane, 
SelectBox, Slider, SplitPane, Stack, Window, TextButton, TextField, 
Touchpad, and Tree.
Scene2D UI also supports the easy creation of new custom widgets. We will 
discuss a selected number of widgets as we implement them in our menu. 
For a complete list and description of each widget, check out the official 
documentation at https://code.google.com/p/libgdx/wiki/scene2dui/.
In addition to Scene2D UI, Libgdx also incorporates a separate project called 
TableLayout. TableLayoutmakes it very easy to create and maintain dynamic 
(read: resolution-independent) layouts using tables and also provides an intuitive 
API. A class called Tableprovides access to the functionality of TableLayout, which 
is also implemented as a widget, and therefore integrates seamlessly into the concept 
of Scene2D UI.
It is highly recommended to check out the official documentation at 
https://code.google.com/p/table-layout/.
One more important feature of Scene2D UI is the support of skins. A skin is a 
collection of resources which are used to style and display UI widgets. For example, 
resources can be texture regions, fonts, and colors. Typically, a skin uses texture 
regions which come from a texture atlas. The style definition of each widget is 
stored in a separate file using the JSON file format.
